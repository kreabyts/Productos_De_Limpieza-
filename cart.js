...
// ============================================================
// GENERADOR DE RECIBO EN IMAGEN
// ============================================================
function generateAndDownloadReceipt(callback) {
    if (typeof html2canvas === 'undefined') {
        if (callback) callback();
        return;
    }

    if (typeof showToast === 'function') showToast("Generando recibo de compra...", "info");
    ...
    cart.forEach((item, index) => {
        const uPrice = getItemUnitPrice(item);
        const subtotal = uPrice * item.qty;
        const bgClass = index % 2 === 0 ? 'bg-white' : 'bg-purple-100'; // Filas alternas
        
        tbody.innerHTML += `
            <tr class="${bgClass}">
                <td class="border border-indigo-900 py-2 px-3 text-center font-bold text-slate-700">${item.qty}</td>
                <td class="border border-indigo-900 py-2 px-3 text-left font-black text-indigo-900">${item.name}</td>
                <td class="border border-indigo-900 py-2 px-3 text-right font-black text-indigo-900">Bs. ${subtotal.toFixed(2)}</td>
            </tr>
        `;
    });
    ...
    html2canvas(captureArea, {
        ...
        link.download = `Pedido_Kreabyts_${now.getTime()}.jpg`;
        link.href = canvas.toDataURL('image/jpeg', 0.9);
        link.click();
        
        setTimeout(() => {
            if (callback) callback();
        }, 800); // Pequeña pausa para asegurar la descarga antes de abrir WhatsApp
    }).catch(err => {
